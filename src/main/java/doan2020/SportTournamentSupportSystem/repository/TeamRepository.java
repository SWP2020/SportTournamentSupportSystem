...
public interface TeamRepository extends JpaRepository<TeamEntity, Long> {
	...
	@Query(value = "select t.* from teams t left join competitions c on t.competition_id = c.id "
			+ "where c.tournament_id = ?1 and t.status=\'?2\'", nativeQuery = true)
	Collection<TeamEntity> findByCompetitionTournamentIdAndStatus(Long tournamentId, String status);
